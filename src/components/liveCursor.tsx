...
export default function LiveCursor() {
    ...
    const spriteImage = isClient && resolvedTheme === "dark" ? "/oneko-dark.gif" : "/oneko.gif";
    ...
    useEffect(() => {
        if (nekoRef.current && isClient) {
            ...
            if (nekoRef.current) {
                nekoRef.current.style.backgroundImage = `url(${spriteImage})`;
            }
        }
    }, [isClient, spriteImage]);
        ...
        const handleIdle = () => {
            ...
            switch (idleAnimation) {
                case "sleeping":
                    ...
                    setSpriteFrame("sleeping", Math.floor(idleAnimationFrameRef.current / 4));
    ...
    // Update sprite image when theme changes
    useEffect(() => {
        if (nekoRef.current) {
            nekoRef.current.style.backgroundImage = `url(${spriteImage})`;
        }
    }, [resolvedTheme, spriteImage]);
